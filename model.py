...
class deepLPM(nn.Module):
    def __init__(self, adj_norm):
        ...
        self.mu_k = nn.Parameter(torch.FloatTensor(np.random.multivariate_normal(np.zeros(args.hidden2_dim),
                                                                                 np.eye(args.hidden2_dim), args.num_clusters)), requires_grad=False)  # dim=2: [0, 0], [[1, 0,], [0, 1]]
    ...
    def pretrain(self, X, adj_label, labels, labelC):
        if not os.path.exists('./pretrain_model_stop.pk'):
            ...
            #            kmeans.cluster_centers_[:, 1], color='black', s=50)
            # ax.scatter(self.mu_k[:, 0], self.mu_k[:, 1], color='purple', s=50)
            ...
            # self.mu_k.data = torch.from_numpy(kmeans.cluster_centers_).float()

            ################### Hierarchical clustering ########################
            # clustering = AgglomerativeClustering(linkage='ward', n_clusters=3)
            # clustering.fit(Z)
            # labelh = clustering.labels_
            # print("pretraining ARI Hierarchical:", adjusted_rand_score(labels, labelh))
            #
            # from sklearn.neighbors.nearest_centroid import NearestCentroid
            # y_predict = clustering.fit_predict(Z)
            # clf = NearestCentroid()
            # clf.fit(Z, y_predict)
            # print(clf.centroids_)
            #
            # self.mu_k.data = torch.from_numpy(clf.centroids_).float()
            #
            # self.gamma.fill_(1e-16)
            # seq = np.arange(0, len(self.gamma))
            # positions = np.vstack((seq, labelh))
            # self.gamma[positions] = 1.
            ...
            if adjusted_rand_score(labels, labelk) < 0.02:
                deepLPM.pretrain(self, X, adj_label, labels, labelC)
