...
class Load_Data():
    ...
    def Graph(self):
        ...
        return Features, Labels, Adjacency, Edges

    def Simu(self):
        path = './Datasets/Graph_Datasets/{}/'.format(self.dataset)

        Adjacency = np.loadtxt(path + 'Adj.txt')

        Features = np.ones(Adjacency.shape)
...
class Graph_Construction:
    # Input: n * d.
    def __init__(self, X, alpha, beta, Edges, type_of='projection'):  # , alpha, beta, Edges
        self.X = X
    ...
    def Middle(self):
        if self.type_of == 'projection':
            ...
            Graph_middle = torch.sigmoid(Inner_product + self.alpha + torch.matmul(self.Edges, self.beta).squeeze(-1))   # + torch.matmul(self.Edges, self.beta).squeeze(-1)
            ...
            Graph_middle = torch.sigmoid(self.alpha - (tnp - 2*Inner_product + tnp.T) + torch.matmul(self.Edges, self.beta).squeeze(-1))  # + torch.matmul(self.Edges, self.beta).squeeze(-1)
            return Graph_middle
