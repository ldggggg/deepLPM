...
A = np.loadtxt('adj_simuB_3clusters.txt')
labels = np.loadtxt('label_simuB_3clusters.txt')
...
import pylab as pl
def show_graph_with_labels(adjacency_matrix,labels,node_size=20,scale_pos=5.):
    ...
    # nx.draw(gr, node_size=50, alpha=1, width=0.5, node_color=labelC, edge_color='grey')
    pl.axis('off')
    pl.tight_layout()
    pl.show()
...
# y6 = np.array([0.6562, 0.69195, 0.6838, 0.69766, 0.78144, 0.8256, 0.82774, 0.84321]) # mini batch
...
ax.errorbar(x, y4, yerr=asymmetric_error4, fmt='gs--', label='VGAE_kmeans', marker = 'p',
            linewidth=1,
            elinewidth=0.6,# width of error bar line
...
# ax.errorbar(x, y6, yerr=asymmetric_error6, fmt='rs--', label='deepLTRS', marker = 'o',
#             linewidth=1,
#             elinewidth=0.6,# width of error bar line
#             ecolor='r',    # color of error bar
#             capsize=5,     # cap length for error bar
...
ax.set_xlabel('Rate of proximity (Delta)', fontsize=18)
plt.legend(loc='upper left', fontsize=12)
plt.show()
fig.savefig("C:/Users/Dingge/Desktop/results/deepLSM_ScA.pdf", bbox_inches='tight')
...
# example data B
x = np.array([0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0])
y1 = np.array([0.37462, 0.31669, 0.52185, 0.45556, 0.37875, 0.48112, 0.5607, 0.91873])  # SBM
y11 = np.array([0.20605, 0.64443, 0.86948, 0.95016, 0.99624, 0.99925, 1, 1])  # SBM_Remi
y2 = np.array([0.1209, 0.64265, 0.76756, 0.8892, 0.92811, 0.94018, 0.98401, 0.99778])  # our_gamma
# y3 = np.array([0.90081, 0.85165, 1])  # our+kmeans
y4 = np.array([0.02769, 0.38598, 0.59172, 0.75401, 0.85077, 0.94593, 0.98099, 0.99922])  # VGAE+kmeans
y5 = np.array([0.20463, 0.34548, 0.43935, 0.54027, 0.63283, 0.6201, 0.65147, 0.77354])  # LPCM

# error bar values w/ different -/+ errors that
# also vary with the x-position
lower_error1 = np.array([0.0022, 0.5321, 0.607, 0.6826, 0.9849, 0.9925, 1, 1])
upper_error1 = np.array([0.4964, 0.7586, 0.9778, 1, 1, 1, 1, 1])
...
lower_error4 = np.array([0.0081, 0.2305, 0.4839, 0.7032, 0.7756, 0.9045, 0.9533, 0.9922])
upper_error4 = np.array([0.0448, 0.5048, 0.6511, 0.82, 0.9351, 0.9783, 1, 1])
lower_error5 = np.array([0.094, 0.2945, 0.3683, 0.4621, 0.5755, 0.5714, 0.5847, 0.6029])
upper_error5 = np.array([0.2655, 0.3803, 0.4795, 0.595, 0.674, 0.668, 0.6887, 1])
...
plt.show()
fig.savefig("C:/Users/Dingge/Desktop/results/deepLSM_ScB.pdf", bbox_inches='tight')
...
plt.xlabel('Different latent dimension', fontsize=15)
plt.ylabel('Clustering ARI', fontsize=15)
...
# plt.ylabel('Training loss + lnK!', fontsize=15)
# plt.show()
f.savefig("C:/Users/Dingge/Desktop/results/diff_clus_eveques.pdf", bbox_inches='tight')
